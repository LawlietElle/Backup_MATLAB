...
inversa = inv(A);
err = norm(inversa-inversa_c,inf)/norm(inversa,inf)
%per verificare che le due matrici siano
%numericamente equivalenti, verifico che
%l'errore relativo in norma infinito, per esempio, 
...
for n = 1000:1000:5000   %100:100:500
    ...
    y = L\(P*b);
    x = U\y;
    err1(i) = norm(ones(n,1)-x)/norm(ones(n,1));
    t1 = toc;
    tic
    [Q,R] = qr(A);
    x = R\(Q'*b);
    err2(i) = norm(ones(n,1)-x)/norm(ones(n,1));
    t2 = toc;
    r(i) = t2/t1;
    i = i+1;
end
